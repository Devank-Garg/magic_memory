...
import re
from memory import core_memory
...
def parse_and_apply(user_id: str, response_text: str) -> tuple[str, list[str]]:
    ...
    for cmd_type, cmd_value in commands:
        ...
        if cmd_type == "REMEMBER":
            core_memory.update_fact(user_id, cmd_value)
            actions.append(f"📌 Remembered: {cmd_value}")
            
        elif cmd_type == "NOTE":
            core_memory.update_scratch(user_id, cmd_value)
            actions.append(f"📝 Note updated: {cmd_value}")
            
        elif cmd_type == "NAME":
            core_memory.set_user_name(user_id, cmd_value)
